...
import { App, Button, Modal, Form, Input, Select, Popconfirm, Space, TreeSelect, Checkbox, Tag } from 'antd';
...
const UserPage: React.FC = () => {
  ...
  // 职位可选范围（教师管理）：仅教职工；主任只有平台管理员可设
  const postOptions = POST_OPTIONS.filter(
    (o) =>
      o.value !== 'student' &&
      o.value !== 'platform_admin' &&
      (isPlatformAdmin || o.value !== 'director'),
  );
      ...
      <ProTable
        ...
        request={async (params) => {
          const res: any = await getUsers({
            ...
            keyword: params.name,
            state: params.state,
            post: params.post || 'teacher,director,deputy',
            orgId: params.orgId,
          });
          return {
          ...
          <Form.Item
            ...
            tooltip={
              isProtectedEdit
                ...
                : isPlatformAdmin
                  ? '调整职位会同时按职位重算默认权限'
                  : '仅可创建/调整本教研室范围内的职位（学生/教师/主任/副主任）'
            }
          >
            <Select
